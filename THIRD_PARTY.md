...
This inventory was reviewed on **2026-07-22**. Exact direct pins come from `pyproject.toml`;
...
versioned subprocess bridge to the Python scientific service. SceneKit displays verified brain,
probe, and vessel geometry; scientific coordinate conversion and analysis remain in Python. The
former PySide6/PyVista/VTK application was removed from the package and lockfile after the Phase 1
reachability audit.
...
| Renier, Nicolas; de Launoit, Elisa; Skriabine, Sophie (2026), *Vascular graphs of the developing post-natal mouse brain*, record DOI `10.5281/zenodo.18876865` | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) | Bundled display derivative of specimen P60_606: maximal consecutive in-bounds graph runs with point radius at least 15 µm. NPZ is 814,393 bytes, SHA-256 `fb2344e845e604be3424bd63f4222d273eafba34db0df2eaff32f4400fa9afec`. The 5,050,194,723-byte source archive and 12,282,574,483-byte extracted graph are not bundled. This fixed/cleared reference omits pial, choroidal, and filtered smaller vessels; it is not subject-specific and cannot establish clearance or trajectory suitability. | Data — bundled derived; [versioned Zenodo record](https://zenodo.org/records/18876865), [Cell paper](https://doi.org/10.1016/j.cell.2026.03.013), [derivation record](docs/LAMBADA_MAJOR_VESSELS.md) |
...
The bundled LAMBADA P60_606 derivative is likewise not relicensed by this application's
source-code license. Preserve its CC BY 4.0 attribution, versioned record DOI, source and asset
digests, adjacent manifest, and fixed/cleared single-specimen limitations in every distributed
build and scientific output that uses the layer.
...
4. Keep the LAMBADA derivative and manifest together and preserve its exact attribution,
   identity, extraction rule, and limitations.
5. Preserve the NP1 model's pinned source identities/digests and review-pending status until a
   separately recorded independent review is complete; do not bundle upstream documents or code
   without a new terms review.
6. Re-review this file when a dependency, atlas version, packaging mode, copied asset, or
